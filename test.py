...
def batched_predict(model, inp, coord, cell, bsize):
    with torch.no_grad():
        ...
        while ql < n:
            qr = min(ql + bsize, n)
            pred = model.query_texture(coord[:, ql: qr, :], cell[:, ql: qr, :])
            preds.append(pred)
            ql = qr
...
def eval_psnr(loader, model, data_norm=None, eval_type=None, eval_bsize=None, window_size=0, scale_max=4, fast=False,
    ...
    elif eval_type.startswith('histo'):
        # scale = int(eval_type.split('-')[1])
        scale = 2
        metric_fn = partial(utils.calc_psnr, dataset='histo', scale=scale)
    ...
    for batch in pbar:
        ...
        if eval_type is not None and fast == False:  # reshape for shaving-eval
            ...
            predsave = img_as_ubyte(np.clip(pred.cpu().numpy()[0], 0, 1)).transpose((1, 2, 0))
        name = batch[k]
#       io.imsave('/root/autodl-tmp/tcgatextureattensave/X2/' + name[0], predsave)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='/data/minghongduan/githubsave/ISTE/configs/test/test-histo-2.yaml')
    parser.add_argument('--model', default='/data/minghongduan/githubsave/ISTE/save/_train_swinir-lte-liif/epoch-last.pth')
